...
public class ContextLoader {
	...
	static {
		...
		// 从 ContextLoader.properties 配置文件中读取默认实现类
		// 根据以上静态代码块的内容，我们推断在当前类 ContextLoader
		// 同样目录下必定会存在属性文件ContextLoader.properties，查看后果然存在
		try {
	 ...
	 *
	 * 这个加载器管理的根 WebApplicationContext 实例。
	 */
	@Nullable
	...
	public WebApplicationContext initWebApplicationContext(ServletContext servletContext) {
		...
		// 下面这个判断 是在调用该方法之后才set 里面的，如果刚进入的时候 就已经有该属性  则认为是存在 多次 定义
		...
		try {
			...
			if (this.context instanceof ConfigurableWebApplicationContext) {
				...
				if (!cwac.isActive()) {
					...
					if (cwac.getParent() == null) {
						...
						// 容器是分层级的  所有需要加载父容器
						// BeanFactory 是Spring 容器，容器是分层级的，
						// ApplicationContext 是应用上下文，里面包好了 BeanFactory，获取bean的时候也是通过 容器获取的
					...
					// 设置 cwac 相关属性并调用refresh
					configureAndRefreshWebApplicationContext(cwac, servletContext);
				}
			}
			// ServletContext 中存入 WebApplicationContext 对象属性信息
			// Spring通过实现 ServletContextListener 接口，核心目的便是系统启动时初始化
			// WebApplicationContext 实例并存放至 ServletContext 中
			...
			if (ccl == ContextLoader.class.getClassLoader()) {
				...
				// 映射当前的类加载器 与创建的实例 到全局变量 currentContextPerThread 中
	...
	protected WebApplicationContext createWebApplicationContext(ServletContext sc) {
		...
		Class<?> contextClass = determineContextClass(sc);// determine：决定

		if (!ConfigurableWebApplicationContext.class.isAssignableFrom(contextClass)) {
			throw new ApplicationContextException("Custom context class [" + contextClass.getName() +
					"] is not of type [" + ConfigurableWebApplicationContext.class.getName() + "]");
		}
		// 1:首先需要确定具体的实现类类型，毕竟 WebApplicationContext 仅仅只是一个上层接口，
		// 2:之后通过反射创建一个实例即可
	...
	/**
	 *
	 * 判断决定 WebApplicationContext 具体要创建的子类类型
	 *
	...
	protected Class<?> determineContextClass(ServletContext servletContext) {
		...
		// 获取 ServletContext 名称为 “contextClass” 的初始化参数的值
		String contextClassName = servletContext.getInitParameter(CONTEXT_CLASS_PARAM);

		// 如果web.xml中指定了 WebApplicationContext 具体要创建的子类类型，就用指定的，否则采用默认的
	...
	protected void configureAndRefreshWebApplicationContext(ConfigurableWebApplicationContext wac, ServletContext sc) {
		if (ObjectUtils.identityToString(wac).equals(wac.getId())) {
			...
			// 替换 WebApplicationContext 容器的id，起一个更有意义的名字。
			// 如果 ServletContext 配置了则使用配置，否则默认规则起名
		...
		// 设置Spring容器的配置文件路径  contextConfigLocation
		...
		// 这个Web容器的上下文刷新  还是沿用的之前的 AbstractApplicationContext中的 refresh 中的方法
