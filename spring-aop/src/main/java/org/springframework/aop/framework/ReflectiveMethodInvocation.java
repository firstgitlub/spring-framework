...
public class ReflectiveMethodInvocation implements ProxyMethodInvocation, Cloneable {
	...
	/**MethodInterceptor和InterceptorAndDynamicMethodMatcher的列表这需要动态检查。
	...
	/**
	 *在 proceed() 方法中，代码逻辑没有想的那么复杂，
	 * ReflectiveMethodInvocation 维护了一个链接调用的计数器，
	 * 记录着当前调用链接的位置，以便链可以有序的进行下去。
	 * 在方法中并没有我们想的那样维护各种增强的顺序，
	 * 而是将此工作分散给了各个具体的增强器，在各个具体的增强器的内部实现
	 */
	//就利用 ReflectiveMethodInvocation 对链进行封装，
	// 在 ReflectiveMethodInvocation 类中 proceed() 方法中实现了拦截器的逐一调用

	// 实现拦截器的逐一调用
	@Override
	@Nullable
	public Object proceed() throws Throwable {
		// We start with an index of -1 and increment early.  我们从一个索引-1开始，并提前递增。

		// 执行完成所有增强方法后 执行 切点方法,就是调用链中的所有拦截器
		// 增强等都被执行完后 执行下面这段代码  {@link https://blog.csdn.net/bskfnvjtlyzmv867/article/details/83242994}
		if (this.currentInterceptorIndex == this.interceptorsAndDynamicMethodMatchers.size() - 1) {
			return invokeJoinpoint();// 进入这里面就代表没有拦截器
		}
