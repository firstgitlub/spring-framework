...
class CglibAopProxy implements AopProxy, Serializable {
	...
	public Object getProxy(@Nullable ClassLoader classLoader) {
		...
		try {
			...
			// Validate the class, writing log messages as necessary.  验证这个类，必要时写日志消息。
			// 验证class
		...
		public Object intercept(Object proxy, Method method, Object[] args, MethodProxy methodProxy) throws Throwable {
			...
			try {
				...
				if (chain.isEmpty() && Modifier.isPublic(method.getModifiers())) {
					...
					/**
					 * 上述的实现与JDK方式实现代理中的 invoke() 方法大同小异，
					 * 都是首先构造链，然后封装此链进行串联调用，
					 * 稍有些区别就是在JDK中直接构造 ReflectiveMethodInvocation，
					 * 而在CGLIB中使用 CglibMethodInvocation。
					 * CglibMethodInvocation 继承自 ReflectiveMethodInvocation，
					 * 但是 proceed() 方法并没有重写。
					 */
	...
	private static class CglibMethodInvocation extends ReflectiveMethodInvocation {
		...
		/**  与使用反射相比，提供了一个边际性能改进 在调用公共方法时调用目标。
