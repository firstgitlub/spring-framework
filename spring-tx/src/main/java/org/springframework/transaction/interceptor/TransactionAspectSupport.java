...
public abstract class TransactionAspectSupport implements BeanFactoryAware, InitializingBean {
	...
	protected Object invokeWithinTransaction(Method method, @Nullable Class<?> targetClass,
		...
		if (txAttr == null || !(ptm instanceof CallbackPreferringPlatformTransactionManager)) {
			// Standard transaction demarcation with getTransaction and commit/rollback calls.  使用getTransaction和提交/回滚调用的标准事务划分。
			// 开启事务  获取数据库连接
			...
			Object retVal;
			try {
				// 这里是准备事务  调用orm框架中的 crud 方法
				// This is an around advice: Invoke the next interceptor in the chain.
				// This will normally result in a target object being invoked.
				// 这里写的是undo log 和 redo log
				// 5. 执行被增强 方法  调用orm框架中的 crud 方法
				// 这一部分在AOP中已经分析过，主要是进行事务拦截器调用完就执行目标本身方法。
				retVal = invocation.proceedWithInvocation();
			}
			catch (Throwable ex) {
				// target invocation exception
				// 回滚事务 写的是 redo ，依据是undo 中记录的 事务开始时候的数据
			...
			// 提交事务  这里面会走真正的事务管理器中的commit方法 里面还做了 根据事务状态进行是否回滚的操作
			// 提交 暂且可以理解为 是日志持久化  也就是 提交事务是写入的 binlog
	...
	protected TransactionInfo createTransactionIfNecessary(@Nullable PlatformTransactionManager tm,
		...
		// txAttr 对应的类型是 RuleBasedTransactionAttribute
	...
	/**
	 *  在调用成功完成后执行，而不是在异常被处理后执行。如果没有创建事务，就什么也不做。
	 *
	...
	protected static final class TransactionInfo {
		...
		private void restoreThreadLocalStatus() {
			...
			// Will be null if none was set.  使用堆栈恢复旧事务。如果没有设置，则为空。  这里是针对事务挂起的情况吧
	...
	private class ReactiveTransactionSupport {
		...
		public Object invokeWithinTransaction(Method method, @Nullable Class<?> targetClass,
			...
			// 对于Mono和暂停功能，不返回kotlinx.coroutines.flow.Flow
