...
public abstract class AbstractPlatformTransactionManager implements PlatformTransactionManager, Serializable {
	...
	/**
	 * !!!!!!!!!!!  下面这句话很重要
	 * { @link https://blog.csdn.net/bskfnvjtlyzmv867/article/details/88787121}
	 *
	 * 脏读、不可重复读都是因为并发事务在修改同一份数据的时候导致的问题，
	 * 这些问题可以通过对同一个记录加锁的方式来解决。
	 * 幻读则是并发事务对不同数据操作时导致的问题，此种情况只能通过表锁、事务的串行化来解决。
	 */
	...
	public final TransactionStatus getTransaction(@Nullable TransactionDefinition definition)
		...
		// 2. 判断当前线程是否存在事务，判读依据为当前线程记录的连接不为空
		// 且连接中(connectionHolder)的transactionAdvice属性不为空
		if (isExistingTransaction(transaction)) {
			// Existing transaction found -> check propagation behavior to find out how to behave.

			// 当前线程已经存在事务，则转向嵌套事务的处理  嵌套事务不是那种 只会滚部分的吗  Nexted 事务的传播特性
			return handleExistingTransaction(def, transaction, debugEnabled);
		}

		// Check definition settings for new transaction.  检查新事务的定义设置。
		// 3. 事务超时设置验证
		...
		// 4. 执行到这说明当前线程不存在事务，但是propagationBehavior却被声明为 PROPAGATION_MANDATORY 抛出异常
	...
	private TransactionStatus startTransaction(TransactionDefinition definition, Object transaction,
		...
		// 7. 新同步事务的设置，针对于当前线程的设置，将事务信息记录在当前线程中
	...
	private TransactionStatus handleExistingTransaction(
		...
		// NEVER不支持事务，只有在没有事务的环境中才能运行(运行到这里说明有事务，抛出异常)
		if (definition.getPropagationBehavior() == TransactionDefinition.PROPAGATION_NEVER) {
			throw new IllegalTransactionStateException(
					"Existing transaction found for transaction marked with propagation 'never'");
		}

		// NOT_SUPPORTED不支持事务，当前事务已经存在，会挂起存在的事务，直至该方法结束后恢复存在的事务
		...
		// NESTED和REQUIRES_NEW有点像，但是如果出错只会回滚新开启的小事务，别的不影响
		if (definition.getPropagationBehavior() == TransactionDefinition.PROPAGATION_NESTED) {
			...
			if (useSavepointForNestedTransaction()) {
				...
				// 如果没有可以使用保存点的方式控制事务回滚，那么在嵌入事务的建立初始新建保存点
	...
	/**
	 * Create a TransactionStatus instance for the given arguments.  为给定的参数创建一个TransactionStatus实例。
	 */
	...
	/**
	 * Initialize transaction synchronization as appropriate.  适当初始化事务同步。
	 */
	...
	private void processCommit(DefaultTransactionStatus status) throws TransactionException {
		try {
			...
			// 在commit之后触发回调，并在那里抛出一个异常传播到调用方，但事务仍被视为已提交。
		...
		try {
			...
			try {
				...
				if (status.hasSavepoint()) {
					...
					// 如果有保存点，也就是当前事务为单独的线程则会退回到保存点。
					// 常用于嵌入式事务回滚，不会引起外部事务回滚
	...
	/** 为提交做准备，在beforeCommit同步回调发生之前执行。请注意，异常将传播到提交调用者，并导致事务回滚。
