...
public interface TransactionSynchronization extends Ordered, Flushable {
	 ...
	 * (note: do not throw TransactionException subclasses here!)
	 *
	 * 事务提交后调用。可以在主事务成功提交后执行进一步的操作。
	 * 例如，可以提交主事务成功提交后应该进行的进一步操作，如确认消息或电子邮件。
	 * 注意:事务将已经提交，但事务资源可能仍然是活动的和可访问的。
	 * 因此，此时触发的任何数据访问代码仍将“参与”到原始事务中，允许执行一些清理(不再有提交)，
	 * 除非它显式地声明需要在单独的事务中运行。因此:对从这里调用的任何事务操作使用PROPAGATION_REQUIRES_NEW。
	 *
	 */
