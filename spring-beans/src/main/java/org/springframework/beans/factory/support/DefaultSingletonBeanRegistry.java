...
public class DefaultSingletonBeanRegistry extends SimpleAliasRegistry implements SingletonBeanRegistry {
			...
			if (singletonObject == null && allowEarlyReference) {
				...
				synchronized (this.singletonObjects) {
					...
					if (singletonObject == null) {
						...
						if (singletonObject == null) {
							/**
							 * 之所以需要从 singletonFactories 中加载，因为在创建单例 bean 的时候会存在依赖注入 的情况，
							 * 而在创建依赖 bean 的时候，为了避免循环依赖，Spring中创建 bean 的原则是不等 bean
							 * 创建完成就会提前将创建 bean 的 ObjectFactory 曝光加入到缓存中，一旦下一个 bean
							 * 创建的时候需要依赖该 bean 时则可以直接使用 ObjectFactory 作为依赖进行注入
							 */
	 ...
	 *
	 * 销毁给定的bean。如果找到相应的可丢弃bean实例，则委托销毁bean。
	 *
	 * @param beanName the name of the bean
	 * @see #destroyBean
	 */
