...
public abstract class PropertyResourceConfigurer extends PropertiesLoaderSupport
	 ...
	 *
	 * 从上图中我们可以看到 PropertyPlaceholderConfigurer 间接的继承了 BeanFactoryPostProcessor 接口。
	 * 这是一个很特别的接口，当Spring加载任何实现了这个接口的 bean 的配置时，
	 * 都会在 bean工厂载入所有 bean 的配置之后执行 postProcessBeanFactory 方法。关
	 * 于 postProcessBeanFactory 方法是在 PropertyResourceConfigurer 类中实现的，
	 * 实现的方法中先后调用了 mergeProperties()、convertProperties()、processProperties() 这三个方法，
	...
	public void postProcessBeanFactory(ConfigurableListableBeanFactory beanFactory) throws BeansException {
		try {
			...
			//正是通过实现 BeanFactoryPostProcessor 接口，
			// BeanFactory 会在实例化任何 bean 之前获得配置信息，从而能够正确的解析 bean 描述文件中的变量引用
