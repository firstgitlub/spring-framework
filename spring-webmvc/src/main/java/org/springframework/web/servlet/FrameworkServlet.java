...
public abstract class FrameworkServlet extends HttpServletBean implements ApplicationContextAware {
	...
	// ServletContext 属性来查找 WebApplicationContext
	@Nullable
	...
	// 将 LocaleContext 和 RequestAttributes 暴露为可继承的子线程
	...
	protected final void initServletBean() throws ServletException {
		...
		try {
			...
			// 创建或刷新 WebApplicationContext 实例(子容器)并对 servlet 功能所使用的变量进行初始化
	 ...
	 * @see #setContextConfigLocation
	 *
	 * 这个方法非常的眼熟，我们在上一部分就是利用该同名方法进行初始化
	 * WebApplicationContext 容器实例的，只不过之前的是在 ContextLoader 类中，
	 * 现在是在 FrameworkServlet 类中
	 *
	 * initWebApplicationContext() 也是初始化一个Spring IOC容器，
	 * 只是这个容器的配置文件加载的Spring MVC配置文件，所以称为Spring MVC容器
	 *
	 *
	 */
	protected WebApplicationContext initWebApplicationContext() {
		...
		/**
		 * 当然这里的 WebApplicationContext 实例是指子容器实例。
		 * 而父容器实例就是这里的 rootContext，当然 rootContext 很有可能为空，
		 * 因为用户很可能没有在 web.xml 中配置父容器的创建，所以只存在Spring MVC的容器。
		 */
		// 从ServletContext中获取父容器 WebApplicationContext，如果没有指定<context-param>，则为空
		...
		// 通过判断 this.webApplicationContext 是否为 null 来知道 this.webApplicationContext
		// 是否是通过构造函数创建的,也就是一启动就创建了
		if (this.webApplicationContext != null) {
			...
			if (wac instanceof ConfigurableWebApplicationContext) {
				...
				// 如果SpringMVC容器还没有刷新，那么就进行刷新父容器的上下文，设置id等操作
				if (!cwac.isActive()) {
		...
		if (wac == null) {
			...
			// 没有被初始化，则查找是否有存在的容器-->根据contextAttribute属性加载WebApplicationContext
			wac = findWebApplicationContext();
		}
		if (wac == null) {
			// No context instance is defined for this servlet -> create a local one
			// 仍然没有的话 DispatcherServlet 就自己创建一个子容器
			wac = createWebApplicationContext(rootContext);
		}

		/**
		 * 回头看 initWebApplicationContext() 的逻辑，
		 * 当执行完 configureAndRefreshWebApplicationContext() 方法后，
		 * 其实容器还没有刷新，this.refreshEventReceived 还是 false，
		 *
		 * 所以不会执行 onRefresh(wac) 方法
		 */
		...
		if (this.publishContext) {
			...
			// 将子容器当作属性设置给ServletContext，
			// 属性名为org.springframework.web.servlet.FrameworkServlet.CONTEXT.+servlet name
	...
	/**
	 *
	 * 通过在 web.xml 文件中配置的 servlet 参数 contextAttribute 来查找 ServletContext 中对应的属性
	 *
	...
	protected WebApplicationContext createWebApplicationContext(@Nullable ApplicationContext parent) {
		...
		// 通过反射方式实例化 contextClass，创建出一个容器
		ConfigurableWebApplicationContext wac =
				(ConfigurableWebApplicationContext) BeanUtils.instantiateClass(contextClass);

		wac.setEnvironment(getEnvironment());

		// parent为在 ContextLoaderListener 中创建的实例，即为父容器
		// 在 ContextLoaderListener 加载的时候初始化的 WebApplicationContext 类型实例
		wac.setParent(parent);

		// 获取contextConfigLocation属性,SpringMVC的配置文件路径，配置在servlet初始化参数中
	...
	protected final void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		// 对于不同的方法，Spring 并没有做特殊处理，
		// 而是统一将程序再一次地引导至 process Request(request, response)中
		processRequest(request, response);
	}
	 ...
	 *
	 * 这个算不算是一个 处理请求的入口呢？？？  从上层HttpServlet算起，是一个子类的真正的处理入口
	 *
	 *
	 */
	protected final void processRequest(HttpServletRequest request, HttpServletResponse response)
		...
		// 为了保证当前线程的 LocaleContext(为国际化考虑) 以及 RequestAttributes
		...
		try {
			...
			// 打印日志输出结果
			// 打印日志级别是 debug ,如果是info 则不会输出任何日志
	...
	/**
	 *
	 * 监听 ContextRefreshedEvent 事件的监听器
	 *
