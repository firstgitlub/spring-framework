 ...
 *
 * 通过之前的配置文件的定义，可以看到我们将自定义的消息监听器注入进监听器容器中，
 * 这样才可以在收到信息时，容器把消息转向监听器处理。
 *
 * { @link https://wangguoping.blog.csdn.net/article/details/84112213}
 */
public class DefaultMessageListenerContainer extends AbstractPollingMessageListenerContainer {
	 ...
	 * listener thread.
	 *
	 * 常量，指示为每个对象缓存一个共享JMS {@code Connection}
	 * 侦听器线程。
	 *
	 * @see #setCacheLevel
	 */
	 ...
	 *
	 * 常量，指示为每个侦听器线程缓存一个共享JMS连接和一个JMS会话。
	 *
	 * @see #setCacheLevel
	 */
	 ...
	 *
	 * 常量，指示为每个侦听器线程缓存共享 JMS连接 、JMS会话 和 JMS MessageConsumer。
	 *
	 * @see #setCacheLevel
	 */
	...
	/**
	 * 委托 initialize() 方法进行执行。initialize()
	 * 方法其实在三个类中分别进行了实现，所以我们真实调用 initialize() 方法
	 * 必然是从 DefaultMessageListenerContainer 中进入的。
	 *
	 * 实现父类的initialize()方法，在afterPropertiesSet()中调用
	 *
	 * DefaultMessageListenerContainer 其实是一个用于异步消息监听的管理类，
	 * 其核心是包含一个任务执行器，不断的执行任务——消息监听
	 */
	@Override
	public void initialize() {
		...
		// 准备 taskExecutor 和 maxMessagesPerTask
		synchronized (this.lifecycleMonitor) {
		...
		// 做完这两件事 (cacheLevel 和 taskExecutor) 之后，
		// 调用父类 AbstractPollingMessageListenerContainer 的 initialize() 方法
		super.initialize();
	}
	 ...
	 *
	 * 这里触发调用的地方是 ApplictaionContext 中完成上下文刷新的时候 调用了
	 * 实现lifeStyle 接口的bean,进而触发对应bean中的start方法
	 *
	 *  参见：DefaultLifecycleProcessor.startBeans()
	 */
	@Override
	 ...
	 *
	 *
	 *  创建一个默认的 SimpleAsyncTaskExecutor
	 *  SimpleAsyncTaskExecutor不重用任何线程，或者说它每次调用都启动一个新线程。但是，它还是支持对并发总数设限，
	 *  当超过线程并发总数限制时，阻塞新的调用，直到有位置被释放。 ，这个执行器的缺点是不会重用连接
	 *
	 */
	...
	private void scheduleNewInvoker() {
		...
		/**
		 * 之前我们仅仅只是相当于创建了消息监听的容器，容器创建了任务执行器以及要执行的任务，
		 * 但是并没有执行，任务都被加入到 this.pausedTasks 集合中，
		 * 我们在测试代码中也没有发现明显启动线程执行的方法，那么问题来了，Spring究竟如何启动任务执行器执行任务的呢？
		 *
		 * 其实答案在 ApplicationContext 中，之前分析 ApplicationContext 的执行流程时，
		 * 在 refresh() 方法的最后一步是 finishRefresh() 方法。
		 *
	 ...
	 *
	 * 之前我们已经说了消息监听器容器本质是创建了任务执行器 taskExecutor，
	 * 为了执行任务——消息监听。消息监听任务其实就是一个 Runnable 接口的实现类。
	 * Spring将消息监听的任务被抽象成 AsyncMessageListenerInvoker 类，
	 * 这个类实现了 Runnable 接口，内部 run() 方法其实是通过不断循环
	 * MessageConsumer.receive() 方法来实现一直监听
	 *
	 * 消息监听任务，本质是 Runnable
	 *
	 */
	private class AsyncMessageListenerInvoker implements SchedulingAwareRunnable {
		...
		public void run() {
			...
			synchronized (lifecycleMonitor) {
				...
				// 唤醒所有在 lifecycleMonitor 对象锁 上的线程
			...
			try {
				...
				// 之所以两种情况分开处理，正是考虑到在无限制循环接收消息的情况下，
				// 用户可以通过设置标志位 this.running 来控制消息接收的暂停与恢复，并维护当前消息监听器的数量
				if (messageLimit < 0 && idleLimit < 0) {
					...
					while (isRunning() && (messageLimit < 0 || messageCount < messageLimit) &&
						...
						// 这里是关键
						// 其实核心的处理就是调用 invokeListener() 来接收消息并激活消息监听器
		...
		private boolean executeOngoingLoop() throws JMSException {
			...
			while (active) {
				synchronized (lifecycleMonitor) {
					...
					while ((active = isActive()) && !isRunning()) {
						...
						try {
							...
							// 通过wait()让获取lifecycleMonitor对象锁的线程等待，
							// 等待之后获取lifecycleMonitor对象锁的线程notify或者notifyAll
				...
				if (active) {
					// 正常处理流程  还是调用的额 invokeListener() 方法
