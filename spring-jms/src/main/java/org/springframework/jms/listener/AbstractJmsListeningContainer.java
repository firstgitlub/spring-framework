...
public abstract class AbstractJmsListeningContainer extends JmsDestinationAccessor
	...
	public void initialize() throws JmsException {
		try {
			...
			synchronized (this.lifecycleMonitor) {
				// 将active置为true
				this.active = true;
				// 初始化先将 this.active 设置为 true，并将获取到 this.lifecycleMonitor 对象锁，
				// 之后又进入等待 (Waiting) 状态的线程全部唤醒
	...
	protected void doStart() throws JMSException {
		...
		synchronized (this.lifecycleMonitor) {
			// 之后我们明显的看到将 this.running 标记位置为 true，this.lifecycleMonitor
			// 唤醒了所有等待线程，通过 resumePausedTasks() 恢复/启动所有的暂停任务。查看 resumePausedTasks() 方法
			this.running = true;
			this.lifecycleMonitor.notifyAll();
			// resume：重新开始  paused:暂停
			resumePausedTasks();
		}
