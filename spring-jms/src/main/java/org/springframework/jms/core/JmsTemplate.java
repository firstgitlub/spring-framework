...
public class JmsTemplate extends JmsDestinationAccessor implements JmsOperations {
	...
	// 用于与 ConnectionFactoryUtils 交互的内部 ResourceFactory 适配器
	 ...
	 * 发送消息
	 *
	 * @param destination the destination to send this message to  将此消息发送到的目的地
	 * @param messageCreator callback to create a message  回调来创建消息
	 * @throws JmsException
	 */
	@Override
	 ...
	 * @param destination the JMS Destination to send to  发送到的JMS目的地
	...
	protected void doSend(Session session, Destination destination, MessageCreator messageCreator)
		...
		// 根据 destination 创建 MessageProducer (通用方法)
		MessageProducer producer = createProducer(session, destination);
		try {
	...
	protected void doSend(MessageProducer producer, Message message) throws JMSException {
		...
		if (isExplicitQosEnabled()) {
			// DeliveryMode : 交互模式  priority：优先权  timeToLive：在发送时返回消息的生存时间。
	...
	protected Message doReceive(Session session, MessageConsumer consumer) throws JMSException {
		try {
			...
			if (connectionFactory != null) {
				...
				// 这里获取的连接  和 上一步execute()中获取的 有什么不同呢？？？
