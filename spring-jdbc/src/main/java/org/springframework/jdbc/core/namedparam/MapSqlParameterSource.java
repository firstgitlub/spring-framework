 ...
 *
 * 保存给定参数映射的SqlParameterSource实现。
 * 这个类的目的是将参数值的简单映射传递给NamedParameterJdbcTemplate类的方法。
 * 这个类上的addValue方法将使添加几个值变得更容易。这些方法返回对MapSqlParameterSource本身的引用，
 * 因此您可以在一个语句中将几个方法调用链在一起。
 *
