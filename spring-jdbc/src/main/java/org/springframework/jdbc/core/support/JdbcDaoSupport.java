 ...
 *
 * 方便的超类用于基于jdbc的数据访问对象。
 * 需要设置一个DataSource，通过getJdbcTemplate()方法为子类提供基于它的JdbcTemplate。
 * 这个基类主要用于JdbcTemplate的使用，
 * 但也可以在直接使用Connection或使用org.springframework.jdbc.object操作对象时使用。
 *
