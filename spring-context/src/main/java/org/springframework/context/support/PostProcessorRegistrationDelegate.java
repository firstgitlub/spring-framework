...
final class PostProcessorRegistrationDelegate {
	 ...
	 *
	 * @param beanFactory    就是之前步骤初始化出的 beanFactory，AbstractApplicationContext 中的成员变量
	 * @param beanFactoryPostProcessors  由 getBeanFactoryPostProcessors() 方法获得，
	 *                                   也就是 AbstractApplicationContext 中的成员变量 this.beanFactoryPostProcessors，
	 *                                   是一个 ArrayList。这个 list 用于添加 BeanFactoryPostProcessor，
	 *                                   利用 addBeanFactoryPostProcessor() 方法
	 */
	public static void invokeBeanFactoryPostProcessors(
		...
		if (beanFactory instanceof BeanDefinitionRegistry) {
			...
			boolean reiterate = true;  //reiterate :重复地做
			while (reiterate) {
				reiterate = false;
				postProcessorNames = beanFactory.getBeanNamesForType(BeanDefinitionRegistryPostProcessor.class, true, false);
				for (String ppName : postProcessorNames) {
			...
			// 现在，调用 到目前为止处理过的所有处理器的 postProcessBeanFactory回调。
		...
		// 不要在这里初始化FactoryBeans:我们需要离开所有常规bean 未初始化，以让bean工厂后处理器应用于它们
		// Do not initialize FactoryBeans here: We need to leave all regular beans
		// uninitialized to let the bean factory post-processors apply to them!
		...
		// 清除缓存的合并bean定义，因为后处理器可能有 修改原始元数据，例如替换值中的占位符
	...
	public static void registerBeanPostProcessors(
		...
		// 并非重复注册，在beanFactory中会先移除已经存在的BeanPostProcessor
		sortPostProcessors(internalPostProcessors, beanFactory);
		registerBeanPostProcessors(beanFactory, internalPostProcessors);

		// 重新注册后处理器，以检测内部bean为applicationlistener， 将其移动到处理器链的末端(用于拾取代理等)
	...
	/** 当bean在BeanPostProcessor实例化期间被创建时，即当一个bean不符合被所有BeanPostProcessor处理的条件时，记录一条信息消息。
