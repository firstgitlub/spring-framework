...
public abstract class AbstractRefreshableApplicationContext extends AbstractApplicationContext {
	...
	protected final void refreshBeanFactory() throws BeansException {
		...
		try {
			...
			// 创建DefaultListableBeanFactory，XmlBeanFactory继承于该类
			DefaultListableBeanFactory beanFactory = createBeanFactory();

			// 为了序列化指定id，
			// 如果需要的话，让这个BeanFactory从id反序列化到BeanFactory对象
			...
			// 加载bean 定义  解析配置文件 或者扫描配置类
			// 初始化DocumentReader，并进行XML文件的读取和解析
			loadBeanDefinitions(beanFactory);

			// 使用全局变量记录BeanFactory类实例  volatile
	 ...
	 *
	 * 确定此上下文当前是否包含一个bean工厂，即已至少刷新一次且尚未关闭。
	 *
	 */
